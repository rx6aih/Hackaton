...
import {botPost} from "../../Services/OurApi/index.js";
...
const CurrentIncidents = ({reports, chatMessage}) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const result = await botPost({street: "Тверская улица", incident: "ДТП на перекрестке"});
                console.log(result);
            } catch (error) {
                console.error(error);
            }
        ...
        fetchData();
    }, []); // Пустой массив зависимостей, чтобы вызов произошел один раз при монтировании компонента
    ...
    return (
        <div className={"flex flex-col w-full h-fit ml-4 rounded-xl border-2 border-solid border-zinc-200"}>
            <div
                ...
                <p className={"flex items-center justify-center h-full w-full text-xl"}>Текущие инциденты</p>
            </div>
            <div
                className={"flex flex-col h-52 w-full bg-gray-100 border-b-2 border-t-2 border-solid border-zinc-400 rounded-md p-2 items-center mt-4 overflow-y-scroll overflow-x-hidden"}>
                {
                    reports.map(report => (
                        <div key={report.location}
                             ...
                             onClick={() => addInChat(report.location)}>
                            <p>{report.location}</p>
                            <p>{report.title}</p>
                        </div>
                    ))
